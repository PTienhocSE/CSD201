...
public class BSTree {
    ...
    public void visit(Node p) {
        ...
        if (q == null) {
            System.out.println(p.getInfo().getISBN() + ", Parent: null");
        } else {
            System.out.println(p.getInfo().getISBN() + ", Parent: " + q.getInfo().getISBN());
        }
    }
    ...
    public Node visitParent(Node p) {
        ...
        while (q != p) {
            f = q;
            if (q.getInfo().getISBN().compareTo(p.getInfo().getISBN()) > 0) {
                q = q.getLeft();
            } else {
                q = q.getRight();
            }
    ...
    public Node search(Node p, String key) {
        ...
        if (p.getInfo().getISBN().equals(key)) {
            return p;
        } else if (p.getInfo().getISBN().compareTo(key) > 0) {
    ...
    public void insert(Book ISBN) {
        ...
        Node f = null, q = root;
        while (q != null) {
            if (q.getInfo().getISBN().equals(ISBN.getISBN())) {
                System.err.println("ISBN cannot be duplicated...");
                return;
            }
            if (q.getInfo().getISBN().compareTo(ISBN.getISBN()) < 0) {
                f = q;
                q = q.getRight();
            } else {
                f = q;
        ...
        if (f == null) {
            root = p;
        } else if (p.getInfo().getISBN().compareTo(f.getInfo().getISBN()) > 0) {
            f.setRight(p);
        } else {
            f.setLeft(p);
        }
